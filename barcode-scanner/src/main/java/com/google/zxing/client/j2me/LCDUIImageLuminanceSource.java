...
public final class LCDUIImageLuminanceSource extends LuminanceSource {
	...
	static byte toLuminance(int pixel) {
		return (byte) ((((pixel & 0x00FF0000) >> 16) + ((pixel & 0x0000FF00) >> 7) + (pixel & 0x000000FF)) >> 2);
	}

	/**
	 * A variant on {@link LCDUIImageLuminanceSource} that acts as if the input is rotated 90 degrees counter-clockwise.
	 */
